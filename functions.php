...
function sandbox_show_loc($print=true){
    $ret = "";
    is_page()        ? $ret = the_title()  : null; // For the front page, if set
    is_home()        ? $ret = 'blog'       : null; // For the blog posts page, if set
    is_single()      ? $ret = 'blog'       : null; // single posts
    is_archive()     ? $ret = 'archive'    : null;
    is_date()        ? $ret = 'date'       : null;
    is_search()      ? $ret = 'search'     : null;
    is_404()         ? $ret = '404'        : null;

    return $print ? print($ret) : $ret;
}
?>